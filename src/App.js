...
const BLOCKCHAIN_LINK = `https://offchainlabs.com/`;
	...
	if (window.ethereum) {
		try {
			...
			await window.ethereum.request({
				...
				params: [{ chainId: '0x421611' }], // Check networks.js for hexadecimal network ids
			});
		} catch (error) {
	  ...
	  try {
		...
		if (ethereum) {
		  ...
		  // Wait for the transaction to be mined
				...
				if (receipt.status === 1) {
					console.log("Domain minted! https://testnet.arbiscan.io/tx/"+tx.hash);
					
					// Set the record for the domain
					tx = await contract.setRecord(domain, record);
					//await tx.wait();
	
					console.log("Record set! https://testnet.arbiscan.io/tx/"+tx.hash);
...
useEffect(() => {
	...
	if (network !== 'Arbitrium Testnet') {
		return (
			<div className="connect-wallet-container">
				<h2>Please switch to Arbitium Testnet</h2>
			...
			<div className="container">
				...
				<img alt="Eth Logo" className="twitter-logo" src={ethLogo} />
					<a
						...
						target="_blank"
						rel="noreferrer"
					>{`Proudly deployed on Arbitrium`}</a>
